...
- (UIView*)buildOverlayView {
    ...
    //overlayView.backgroundColor     = [UIColor colorWithRed:0.0/255.0 green:0.0/255.0 blue:0.0/255.0 alpha:0.8];
    
    // Green header
    CGRect headerBounds = self.view.bounds;
    headerBounds = CGRectMake(0, 0, bounds.size.width, 65);
    UIView* header = [[[UIView alloc] initWithFrame:headerBounds] autorelease];
    header.backgroundColor = [UIColor colorWithRed:171.0/255.0 green:224.0/255.0 blue:80.0/255.0 alpha:1];
    
    // Close button
    UIButton* cancelButtonHeader = [[UIButton alloc] init];
    [cancelButtonHeader setImage:[UIImage imageNamed:@"48_close_icon.png"] forState:UIControlStateNormal];
    [cancelButtonHeader setImage:[UIImage imageNamed:@"48_close_icon_tap.png"] forState:UIControlStateSelected];
    [cancelButtonHeader setFrame:CGRectMake(5, 30, 25, 25)];
    [cancelButtonHeader addTarget:self action:@selector(cancelButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
    [header addSubview:cancelButtonHeader];
    
    // Header title
    UILabel* titleHeader = [[UILabel alloc] init];
    titleHeader.text = @"Scan a Barcode";
    [titleHeader setFont:[UIFont fontWithName:@"Helvetica Neue" size:17]];
    [titleHeader setTextColor:[UIColor whiteColor]];
    CGSize size = [titleHeader.text sizeWithFont:titleHeader.font];
    [titleHeader setFrame:CGRectMake((header.frame.size.width - size.width)/2 , 37, size.width, size.height)];
    [header addSubview:titleHeader];
    
    //    [overlayView addSubview:addText];
    ...
    [reticleView setFrame:CGRectMake(0, header.frame.size.height, self.view.bounds.size.width, self.view.bounds.size.height - header.frame.size.height)];
    reticleView.opaque           = NO;
    
    // Additional title
    UILabel* addText = [[UILabel alloc] init];
    addText.text = @"Point the camera at broduct barcode\nto see product information";
    [addText setFont:[UIFont fontWithName:@"Helvetica Neue" size:15]];
    [addText setTextColor:[UIColor whiteColor]];
    [addText setFrame:CGRectMake(35, 267, 250, 60)];
    addText.lineBreakMode = NSLineBreakByWordWrapping;
    addText.numberOfLines = 0;
    addText.textAlignment = UITextAlignmentCenter;
    [reticleView addSubview:addText];
...
#define RETICLE_WIDTH   self.view.bounds.size.width
#define RETICLE_OFFSET   60.0f
#define RETICLE_ALPHA     0.6f
